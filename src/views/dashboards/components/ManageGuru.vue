<template>
  <div
    class="bg-white p-4 md:p-6 rounded-2xl border border-neutral-200/80 shadow-sm space-y-4 overflow-hidden"
  >
    <div
    ...
    >
      <div>
        <h3 class="text-sm md:text-base font-bold text-neutral-900">
          Daftar Data Master Guru
        </h3>
        <p class="text-xs text-neutral-400">
          Total data terdaftar: {{ listData.length }} orang
        </p>
      </div>
      <Button
        label="Tambah Guru"
        icon="pi pi-plus"
        @click="$emit('add-user')"
        class="p-button-sm bg-neutral-900 text-white border-none font-semibold cursor-pointer text-xs h-10 px-4 rounded-xl w-full sm:w-auto hover:bg-neutral-800 transition"
      />
    </div>
    ...
    <div class="overflow-x-auto w-full rounded-xl border border-neutral-100">
      <DataTable
        ...
        class="p-datatable-minimalist min-w-[600px] w-full"
      >
        <template #empty>
          <div
        ...
        <Column field="username" header="NIP / USERNAME">
          <template #body="slotProps">
            <span
              class="font-semibold text-neutral-800 tracking-tight text-xs"
        ...
        <Column header="AKSI" class="w-24 text-center">
          <template #body="slotProps">
            <div class="flex items-center justify-center gap-1">
              ...
              <button
                type="button"
                @click="konfirmasiHapus(slotProps.data)"
...
const konfirmasiHapus = (dataGuru) => {
  Swal.fire({
    title: "Hapus Data Guru?",
    html: `Apakah kamu yakin ingin menghapus <b>${dataGuru.username}</b>?<br><span style="color: #ef4444; font-size: 0.875rem;">Tindakan ini tidak dapat dibatalkan.</span>`,
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#ef4444", // Merah Destruktif
    ...
    customClass: {
      ...
      confirmButton: "rounded-xl font-bold text-sm px-5 py-2 shadow-sm",
      cancelButton: "rounded-xl font-bold text-sm px-5 py-2",
    },
  }).then((result) => {
    if (result.isConfirmed) {
      // Jika Admin klik 'Ya, Hapus', barulah kita kirim sinyal hapus ke AdminDashboard.vue
      emit("delete-user", dataGuru.id);
    }
  });
};
</script>
